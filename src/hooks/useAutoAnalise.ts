// ============================================================
// useAutoAnalise.ts — Hook que orquestra o fluxo automático
...
export function useAutoAnalise() {
  ...
  // ----------------------------------------------------------------
  // Função principal: recebe o arquivo PDF e o projeto_id
  // ----------------------------------------------------------------
  const executarAnalise = useCallback(
    async (arquivo: File, projetoId: string) => {
      ...
      try {
        ...
        atualizar("buscando_regras", "Buscando regras regulatórias...", 30);
        ...
        atualizar(
          "analisando",
          `Analisando ${regras.length} regras com IA... (pode levar até 30s)`,
          50
        );
        ...
        // ETAPA 5: Concluído
        atualizar("concluido", "Análise concluída com sucesso!", 100);
        setResultado(analise);
      } catch (err: any) {
        setErro(err.message || "Erro desconhecido na análise.");
...
async function salvarResultados(projetoId: string, analise: ResultadoAnalise) {
  ...
  // Remove validações antigas deste projeto
  await supabase.from("validacoes").delete().eq("projeto_id", projetoId);

  // Insere as novas validações
