...
Analise o texto do projeto arquitetonico e avalie cada regra abaixo.
Responda APENAS com JSON valido, sem markdown:
{"resultados":[{"codigo":"CODIGO","status":"conforme","justificativa":"motivo curto"}]}
Use status: conforme, nao_conforme, ou nao_aplicavel.
...
export async function analisarComGroq(textoPDF: string, regras: RegraRegulatoria[]): Promise<ResultadoAnalise> {
  if (!OPENROUTER_API_KEY) {
    return { resultados: [], resumo: "", score_geral: 0, erro: "Chave VITE_OPENROUTER_API_KEY nao configurada no Vercel." };
  }
